  ...
  br2_tracking_msgs__msg__PanTiltCommand__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    br2_tracking_msgs__msg__PanTiltCommand * data =
      (br2_tracking_msgs__msg__PanTiltCommand *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!br2_tracking_msgs__msg__PanTiltCommand__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          br2_tracking_msgs__msg__PanTiltCommand__fini(&data[i]);
        }
        free(data);
        return false;
      }
